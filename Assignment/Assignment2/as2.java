...
public class as2 {
    ...
    static int generateRandomPin() {
        int randomPin = (int) (Math.random() * 900) + 100; // uses the math random function to create a random number
                                                           // and make a 3 digit pin
        return randomPin;
    }
    ...
    public static void main(String[] args) {
        ...
        // timer.schedule(new FunctionRunner(),0, 60 * 1000); // schedule the function
        // to run after 1 minute
        ...
        do {
            ...
            if (uc == 1) {
                ...
                } else { // customer entered details and didnt accept terms and condition. move to screen
                ...
                if (customerList.size() > 0) { // checks if there are any customers in the database
                    index = customerMatchPin(customerList, askCustomerPin()); // checks if the customer has entered
                                                                              // correct pin
                    if (index != -1) {// only continues if customer enters correct pin
                        performCustomerOP(customerList.get(index), customerList); // passes the customer in the dynamic
                                                                                  // array to perform operations
                ...
                if (customerList.size() == 0) {
                    System.out.println(
                            "Sorry there are no customers in the database. Please create a new account as a new Customer.");
                }
            }
    ...
    static int CustomerDetailsOP(List<Customer> customerList) { // screen 2
        ...
        if (termsAndConditions()) { // checks if the customer accepts the terms and conditions
            customerList.add(new Customer(name, email)); // creates a new customer object with the customers information
                                                         // if customer accepts terms
    ...
    static void customerTransfer(Customer userOP, List<Customer> customerList) {
        ...
        if (customerList.get(id) != null) { // if the customer id exists
            ...
            choice = userInput(); // takes the input from user as a string
            double d = Double.parseDouble(choice); // parses the input string into a double value
            System.out.println("Press pin to confirm you want to send " + d + " to " + c.getCustomerName() + " with id "
                    + c.getCustomerId());
...
class Account {
    ...
    void adjustLoan() {
        if (loan == true) {
            ...
            loan_amount = loan_amount - (loan_rate * loan_amount); // adjusts the value of loan amount after deducting
                                                                   // from balance
        }
    }
